        ...
        for dataset in self.datasets:
            ...
            for error_metr in self.error_metrics:
                for count in self.counts:
                    ...
                    graphics.line_plot(np.array(self.es), np.array(y), xlabel='$\epsilon$', ylabel= error_metr, ylog=True, line_legends=legends, figsize=(7, 5), path=path_result)

                    path_result2 = os.path.abspath(os.path.join(os.path.dirname( __file__ ), 'results', '%s_%s_%s_result.png' % ( dataset, count, error_metr) ))  
                    graphics.line_plot(np.array(self.es), np.array(y), xlabel='$\epsilon$', ylabel= error_metr, ylog=False, line_legends=legends, figsize=(7, 5), path=path_result2)
